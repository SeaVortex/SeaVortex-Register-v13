...
module.exports = {
    name: 'Woman',
    description: '',
    aliases: ['kız', 'woman', 'k', 'w'],
    usage: "kız @user/ID İsim Yaş",
    cooldown: 5,
    ...
    async execute(client, message, args, embeds, timeout) {
        ...
        await user.roles.remove(vortex.roles.man).catch(e => { });
        ...
        if(!regData) {
            let newRegData = new reg({
                ...
                womanReg: 0,
                manReg: 0,
                userNames: [{ nick: `${tag} ${age} ${nick.charAt(0).toUpperCase() + nick.slice(1).toLowerCase()}${age ? ` ${vortex.panels.nametag} ${age}` : ``}`, type: `KIZ`}]
            }).save();
        } else {
